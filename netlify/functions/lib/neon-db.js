...
const userHelpers = {
  ...
  async updateUserStats(userId, sessionData) {
    ...
    try {
      ...
      // Calculate weekly minutes - handle weekly reset
      const now = new Date();
      const lastWeekReset = user.last_week_reset ? new Date(user.last_week_reset) : null;
      const startOfWeek = new Date(now);
      startOfWeek.setDate(now.getDate() - now.getDay()); // Get Sunday of current week
      startOfWeek.setHours(0, 0, 0, 0);
      
      let newWeeklyMinutes = (user.weekly_minutes || 0) + sessionData.duration;
      
      // Reset weekly minutes if it's a new week (Sunday)
      if (!lastWeekReset || lastWeekReset < startOfWeek) {
        newWeeklyMinutes = sessionData.duration; // Start fresh for new week
      }
      ...
      // Update user stats including weekly minutes
      const updatedUser = await client.query(
        `UPDATE users SET 
         ...
         WHERE id = $8 
         RETURNING *`,
        [newExperience, newLevel, newTotalSessions, newTotalMinutes, newWeeklyMinutes, newStreak, (!lastWeekReset || lastWeekReset < startOfWeek), userId]
      );
    ...
    try {
      const result = await client.query(
        `SELECT id, display_name, profile_picture, total_minutes, total_sessions, 
                current_streak, level, experience, weekly_minutes
         FROM users 
         WHERE total_sessions > 0 
